...
$datee=date("Y-m-d H:i:s",strtotime($datt));
//$choice=$_POST["choice"];
...
echo"<br/>'$route[$small]'<br/> $date<br/>$pur <br/> '$planID'<br/>'$username'<br/>'$tourGuideID'<br/>'$agent'<br/>'$route[$small]'<br/>'$nam'";
$sq="INSERT INTO 'plan'('planId', 'touristId', 'tourGuideID', 'tour_TravelAgent ' , 'placeSequence', 'plan_name')
 ...
 $query = "INSERT INTO plan(planId, touristId, tourGuideID, tour_TravelAgent , placeSequence,dateStarted, dateFinished, plan_name, purpose) VALUES (?,?,?,?,?,?,?,?,?)";
$stmt = mysqli_prepare($conn, $query) or die("  parsing error ".mysqli_error($conn));
$stmt->bind_param("sssssssss",$planID, $username , $tourGuideID ,$agent , $m, $date ,$datee, $nam, $pur);
$add=$stmt->execute();
...
if(!$add){
	...
	$d="<br/>failed to add the plan <br/>".mysqli_error($conn);
	...
	$url="http://localhost/ethioTourism.com/users/viewplan.php?uname=$username";
		 header("Location:$url   ");
